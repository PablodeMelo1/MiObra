...
function LoginPage() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const text = error.response?.data?.message ?? "Error al iniciar sesión";
      setMessage("");
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-zinc-800 text-white px-4">
      <form onSubmit={onSubmit} className="w-full max-w-md bg-zinc-900/60 backdrop-blur-sm p-6 rounded-lg shadow-md">
        ...
        <button type="submit" className="w-full mt-4 bg-blue-600 hover:bg-blue-700 text-zinc-900 font-medium py-2 rounded-md">Iniciar sesión</button>
        <a href="/register" className="block mt-4 text-center text-sm text-white/70 hover:text-white">¿No tienes una cuenta? Regístrate</a>
      </form>
    </div>
  )
}
