...
export const createComment = async (req, res) => {
    try {
        ...
        const created = await commentRepository.createOne({ companyId: req.companyId, entityType, entityId, userId, comment });
...
export const getCommentsByEntity = async (req, res) => {
    try {
        ...
        const comments = await commentRepository.getByEntity({ entityType, entityId, companyId: req.companyId });
...
export const deleteComment = async (req, res) => {
    try {
        ...
        const comment = await commentRepository.getById({ _id: id, companyId: req.companyId });
        if (!comment) {
        ...
        const deleted = await commentRepository.deleteById({ _id: id, companyId: req.companyId });
        if (!deleted) {
...
export const updateComment = async (req, res) => {
    try {
        ...
        const updated = await commentRepository.updateById({ _id: id, companyId: req.companyId }, { comment: newComment });
        if (!updated) {
